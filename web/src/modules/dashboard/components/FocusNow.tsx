...
function FocusTaskRow({ task }: { task: FocusTask }) {
  ...
  return (
    <li className="flex items-center gap-3 border border-zinc-800 bg-zinc-900/80 px-3 py-3 transition-colors hover:border-zinc-700">
      <button
        type="button"
        className="shrink-0 text-zinc-600 transition-colors hover:text-emerald-500"
      ...
      <div className="min-w-0 flex-1">
        <Link
          to={`/w/${task.workspaceId}/db/${task.databaseId}`}
          className="block truncate font-medium text-zinc-100 hover:text-emerald-400"
        >
          {task.title}
        </Link>
        <div className="mt-1 flex flex-wrap items-center gap-2">
          {task.focusToday ? (
            <span className="font-mono text-[9px] uppercase text-amber-500/90">
              foco
            </span>
          ) : null}
          {task.dueToday ? (
      ...
      {task.points > 0 ? (
        <span
          className={cn(
            "shrink-0 border px-1.5 py-0.5 font-mono text-[9px] uppercase",
...
export function FocusNow({ data }: FocusNowProps) {
  ...
  return (
    <section className="relative border border-emerald-900/40 bg-zinc-950">
