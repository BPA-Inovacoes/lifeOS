...
export function HabitStreakBadge({
  ...
  if (streak <= 0) {
    return (
      <span className={cn("shrink-0 font-mono text-[10px] text-zinc-700", className)}>
        —
      </span>
    );
  }
  ...
  return (
    <span
      className={cn(
        "flex shrink-0 items-center gap-1 border border-amber-900/50 px-2 py-1 font-mono text-[10px] uppercase text-amber-500",
        className
      )}
      title={
    ...
    >
      ...
      {bestStreak && bestStreak > streak ? (
        <span className="text-zinc-600">· máx {bestStreak}</span>
      ) : null}
    </span>
  );
}
